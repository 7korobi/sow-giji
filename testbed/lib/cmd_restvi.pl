...
sub CmdRestVIndex {
	...
	# データ処理
	&SetDataCmdRestVIndex($sow);

	# HTTP/HTML出力
	&OutHTMLCmdRestVIndex($sow);
}

#----------------------------------------
# データ処理
#----------------------------------------
sub SetDataCmdRestVIndex {
	my $sow = $_[0];
	my $query  = $sow->{'query'};
	my $debug = $sow->{'debug'};
...
sub OutHTMLCmdRestVIndex {
	...
	require "$cfg->{'DIR_HTML'}/html_restvi.pl";
	...
	&SWHtmlRestVIndex::OutHTMLRestVIndex($sow);
